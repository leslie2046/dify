...
class TestRetrievalServiceInternals:
    ...
    def test_retrieve_with_attachment_ids_only(self, monkeypatch: pytest.MonkeyPatch, internal_dataset):
        with (
            ...
            def side_effect(
                ...
                attachment_id=None,
                disable_internal_threading=False,
            ):
    ...
    def test_retrieve_runs_inline_when_internal_threading_is_disabled(self, internal_dataset):
        with (
            patch("core.rag.datasource.retrieval_service.RetrievalService._get_dataset", return_value=internal_dataset),
            patch("core.rag.datasource.retrieval_service.RetrievalService._retrieve") as mock_retrieve,
            ...
            RetrievalService.retrieve(
                ...
                dataset_id=internal_dataset.id,
                query="query",
                disable_internal_threading=True,
            )

        mock_executor.assert_not_called()
        mock_retrieve.assert_called_once()
        assert mock_retrieve.call_args.kwargs["disable_internal_threading"] is True
    ...
    def test_retrieve_internal_runs_searches_inline_when_internal_threading_is_disabled(
        self, internal_dataset, internal_flask_app
    ):
        with (
            patch("core.rag.datasource.retrieval_service.RetrievalService.keyword_search") as mock_keyword_search,
            patch("core.rag.datasource.retrieval_service.ThreadPoolExecutor") as mock_executor,
        ):
            RetrievalService()._retrieve(
                flask_app=internal_flask_app,
