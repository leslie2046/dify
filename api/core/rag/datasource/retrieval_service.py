...
class RetrievalService:
    ...
    def retrieve(
        ...
        execution_metadata: dict[str, float] | None = None,
    ):
        ...
        with flask_app.app_context():
            try:
                ...
                if query_type == QueryType.TEXT_QUERY:
                    ...
                    if execution_metadata is not None:
                        init_latency = getattr(vector, "init_latencies", {}).get("embedding_model_init", 0)
                        if init_latency:
                            execution_metadata["embedding_model_init_latency"] = init_latency
                        execution_metadata["embedding_search_latency"] = time.perf_counter() - start
                if query_type == QueryType.IMAGE_QUERY:
                    ...
                    if execution_metadata is not None:
                        ...
                        if init_latency:
                            execution_metadata["embedding_model_init_latency"] = init_latency
                        execution_metadata["embedding_file_search_latency"] = time.perf_counter() - start
                ...
                if documents:
                    if (
                        ...
                        if execution_metadata is not None:
                            execution_metadata["reranking_latency"] = time.perf_counter() - rerank_start
                    else:
                        all_documents.extend(documents)
            except Exception as e:
                logger.error(e, exc_info=True)
    ...
    def _retrieve(
        ...
        with flask_app.app_context():
            ...
            if retrieval_method == RetrievalMethod.HYBRID_SEARCH:
                ...
                if execution_metadata is not None:
                    execution_metadata["reranking_latency"] = time.perf_counter() - start
