...
class _InlineExecutor(concurrent.futures.Executor):
    """Executor adapter that runs submitted work immediately on the caller thread."""

    @override
    def submit[**P, R](self, fn: Callable[P, R], /, *args: P.args, **kwargs: P.kwargs) -> concurrent.futures.Future[R]:
...
class RetrievalService:
    ...
    def retrieve(
        ...
        disable_internal_threading: bool = False,
    ) -> list[Document]:
        """Retrieve documents for one dataset.

        Disable internal threading when the caller already parallelizes across
        datasets to prevent nested pools from multiplying worker counts.
        """
        ...
        executor = (
            ...
            else ThreadPoolExecutor(max_workers=dify_config.RETRIEVAL_SERVICE_EXECUTORS)  # type: ignore
        )
        with executor:
            futures = []
            ...
            if query:
                futures.append(
                    executor.submit(
                        ...
                        disable_internal_threading=disable_internal_threading,
                    )
                )
            if attachment_ids:
