...
const HitTestingPage: FC<Props> = ({ datasetId }: Props) => {
  ...
  const renderHitResults = (results: HitTesting[] | ExternalKnowledgeBaseHitTesting[]) => (
    <div className="flex h-full flex-col rounded-tl-2xl bg-background-body px-4 py-3">
      ...
      {hitResult?.execution_metadata && !isExternal && (
        <div className="mb-2 flex flex-wrap gap-x-4 gap-y-1 px-2 text-xs text-text-tertiary">
          {Object.entries(hitResult.execution_metadata).map(([key, value]) => (
            <div key={key} className="flex items-center">
              <span className="mr-1 capitalize">{key.replace('_latency', '').replace(/_/g, ' ')}:</span>
